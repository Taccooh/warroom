document.addEventListener('DOMContentLoaded', function () {
  ...
  // ---- Own turf: clan gold or our real gang colour ----
  // Own cells are painted in the app's gold. Some gangs fly a gold close enough
  // to it that own and enemy turf blur into each other on the map — then it
  // helps to drop the brand colour and show our own gang in the colour the feed
  // gives it. Switch sits in the Info tab, remembered per device.
  var GOLD = '#e8b64c';
  var mineRaw = false;
  try { mineRaw = localStorage.getItem('wr_mycolor') === 'raw'; } catch (e) {}
  // Footprint-only cells carry no colour, so hunt for one that does; if the feed
  // never named our gang a colour, gold stays.
  function ownColor() {
    if (!mineRaw) return GOLD;
    for (var i = 0; i < cells.length; i++) {
      if (cells[i].status === 'mine' && cells[i].color) return cells[i].color;
    }
    return GOLD;
  }
  function paintOwn() {
    // The "yours" counter dot reads from --mine, so it follows along
    document.documentElement.style.setProperty('--mine', ownColor());
  }
  paintOwn();
  var mineBtn = document.getElementById('mycolor-toggle');
  function mineBtnText() {
    if (mineBtn) mineBtn.textContent = mineRaw ? T.mycol_raw : T.mycol_gold;
  }
  mineBtnText();
  if (mineBtn) mineBtn.addEventListener('click', function () {
    mineRaw = !mineRaw;
    try { localStorage.setItem('wr_mycolor', mineRaw ? 'raw' : 'gold'); } catch (e) {}
    mineBtnText();
    paintOwn();
    renderCells();
  });
  ...
  function renderCells() {
    ...
    cells.forEach(function (c) {
      ...
      // Enemies in their real gang color (CHAOS vs BWM distinguishable), free cold
      var fill = c.status === 'enemy' ? (c.color || COLOR.enemy)
        : c.status === 'mine' ? own : COLOR.free;
    ...
    fetch('/api/live', {headers: {'X-Requested-With': 'fetch'}})
      ...
      .then(function (d) {
        ...
        if (d.cells && !document.querySelector('.leaflet-popup')) {
          cells = d.cells;
          paintOwn();   // our gang colour rides on the cells, refresh the counter dot
          renderCells();
